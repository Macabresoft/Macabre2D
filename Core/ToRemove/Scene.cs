namespace Macabresoft.MonoGame.Core.ToRemove {
    ...
    public sealed class Scene : NotifyPropertyChanged, IScene, IDisposable {
        ...
        internal FilterSortCollection<CameraComponent> Cameras { get; } = new FilterSortCollection<CameraComponent>(
            c => c.IsEnabled,
            nameof(CameraComponent.IsEnabled),
            (c1, c2) => Comparer<int>.Default.Compare(c1.RenderOrder, c2.RenderOrder),
            nameof(CameraComponent.RenderOrder));
        ...
        private void DrawForCamera(FrameTime frameTime, CameraComponent camera) {
