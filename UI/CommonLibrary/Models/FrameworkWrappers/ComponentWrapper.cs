namespace Macabre2D.UI.CommonLibrary.Models.FrameworkWrappers {
    ...
    public sealed class ComponentWrapper : NotifyPropertyChanged, IHierarchical<ComponentWrapper, IParent<ComponentWrapper>> {
        ...
        public bool RemoveChild(ComponentWrapper child) {
            var result = this._children.Remove(child);
            var scene = this.GetRoot();
            scene.SavableValue.RemoveComponent(child.Component);
            return result;
        }
        ...
        private SceneAsset GetRoot() {
            var component = this;

            while (component.Parent is ComponentWrapper wrapper) {
                component = wrapper;
            }

            return component.Parent as SceneAsset;
        }
