namespace Macabre2D.UI.CommonLibrary.Models {
    ...
    public sealed class ImageAsset : MetadataAsset, ISyncAsset<Sprite>, IIdentifiableAsset {
        ...
        public ObservableCollection<SpriteWrapper> Sprites {
            get {
        ...
        public bool AddChild(SpriteWrapper wrapper) {
            return this.AddChild(wrapper, -1);
        }

        public bool AddChild(SpriteWrapper wrapper, int index) {
            var result = false;
            if (!this.Sprites.Contains(wrapper) && wrapper.ImageAsset == this) {
                ...
                if (index >= 0 && index < this._children.Count) {
                    this._children.Insert(index, wrapper);
                }
                else {
