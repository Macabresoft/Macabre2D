...
public class EditorRenderSystem : RenderSystem {
    ...
    private readonly QuadTree<IScreenSpaceRenderer> _legacyFontRenderTree = new(0, float.MinValue * 0.5f, float.MinValue * 0.5f, float.MaxValue, float.MaxValue);
    ...
    public override void Render(FrameTime frameTime) {
        if (this.Game is IEditorGame { SpriteBatch: { } spriteBatch, Camera: { } camera } sceneEditor) {
            ...
            camera.RenderInScreenSpace(frameTime, spriteBatch, this._legacyFontRenderTree);
